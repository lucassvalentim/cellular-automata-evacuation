...
for exit in exits:
    ...
    shape, _, walls = ScenarioManager.scenario_simulate_exits(exit)
    ff = FloorFieldGenerator.generate(shape, exit, walls)

    collector = MetricsCollector(shape, exit, walls, p_wait=0.5)
    collector.run_batch(
        num_agents=10, 
    ...
    exit_label = next(iter(exit.keys()))
    linha_resultado = {
        'Saida': exit_label,
...
diretorio_saida = "outputs/simulations"
